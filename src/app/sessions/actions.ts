...
export async function registerForSession(sessionId: string) {
  ...
  // Check session is open and in the future
  ...
  if (futureOpenIds.length > 0) {
    ...
    if ((futureRegCount ?? 0) >= 2) return { error: 'כבר רשום ל-2 סשנים עתידיים (כלל המשחק ההוגן)' }
  }
  ...
  // Upsert handles both new registration and re-registration after cancel
